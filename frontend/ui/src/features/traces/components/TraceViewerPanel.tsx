 ...
 * visible on every view — including Detectors, whose overlay covers only the
 * main panel. The main split resizes tree vs details. AI state lives in
 * AiChatProvider above this component, so chat survives trace switching.
 */
      ...
      <div className="flex h-full flex-col bg-background">
        ...
        <div className="relative flex flex-1 overflow-hidden">
          {/* Top-level split: [ main content | AI assistant (optional) ]. The AI
            ...
            view — including Detectors, whose overlay covers only the main panel
            below and so never hides the assistant. */}
          <ResizablePanelGroup orientation="horizontal" className="h-full min-w-0">
            ...
            >
              {/* The detectors tab fetches its own data by traceId, independent
                of the trace fetch, so it overlays the panel group ahead of the
                loading/error/!trace guards — a slow or failed *trace* fetch must
                not hide independently-loaded detector data. It stays mounted
                (hidden when inactive) so its data is already loaded when the tab
                is opened; overlaying — rather than unmounting the group — stops
                the resizable panels re-measuring on toggle. */}
              <div
                className={cn(
                  "absolute inset-0 z-10 bg-background",
                  viewMode !== "detectors" && "hidden",
                )}
              >
                <TraceDetectorsTab projectId={projectId} traceId={traceId} />
              </div>
              {/* Hidden (not unmounted) under the detectors overlay: `invisible`
                keeps the box laid out so the resizable panels don't re-measure,
                while hiding every descendant regardless of its z-index (the
                timeline uses z-20 / the handle z-50, which would otherwise paint
                over the overlay). */}
              <ResizablePanelGroup
                orientation="horizontal"
                className={cn("h-full min-w-0", viewMode === "detectors" && "invisible")}
              >
                {/* LEFT: tree panel */}
                <ResizablePanel
                  id="trace-tree"
